...
export const Route = createRootRoute({
  head: () => ({
    meta: [
      ...
      {
        title: "Java + MyBatis 实战学习 · 系统路径",
      },
      {
        name: "description",
        content:
          "Java MyBatis 中文交互教程：学习路径、讲解+源码+Demo、Catppuccin 主题与 REST 工坊。",
      },
    ],
    links: [
...
const CTP_BOOT =
  "(function(){try{var f=localStorage.getItem('java-mybatis-learn-ctp-flavor');var a=localStorage.getItem('java-mybatis-learn-ctp-accent');var okF=['mocha','macchiato','frappe','latte'];var okA=['green','mauve','blue','lavender','sapphire','teal','peach','pink'];if(okF.indexOf(f)<0)f='mocha';if(okA.indexOf(a)<0)a='sapphire';document.documentElement.setAttribute('data-ctp-flavor',f);document.documentElement.setAttribute('data-ctp-accent',a);}catch(e){document.documentElement.setAttribute('data-ctp-flavor','mocha');document.documentElement.setAttribute('data-ctp-accent','green');}})();";

function RootDocument({ children }: { children: ReactNode }) {
  return (
    <html lang="zh-CN" data-ctp-flavor="mocha" data-ctp-accent="sapphire">
      <head>
...
function AppShell({ children }: { children: ReactNode }) {
  ...
  /* studio uses in-memory mock, no MSW */
