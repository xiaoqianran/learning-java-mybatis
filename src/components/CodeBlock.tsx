...
function colorLine(line: string) {
  ...
  const parts = line.split(
    /(\b(?:import|from|const|let|function|return|export|type|interface|if|else|new|public|private|protected|class|interface|void|int|long|String|List|Map|SELECT|FROM|WHERE|INSERT|UPDATE|DELETE|INTO|VALUES|AND|OR|JOIN|ON|ORDER|BY|LIMIT|SET|CREATE|TABLE)\b|<\/?[A-Za-z][\w.-]*|@[\w.]+|#\{[^}]+\}|\$\{[^}]+\}|'[^']*'|"[^"]*"|`[^`]*`)/g,
  );
  return parts.map((p, i) => {
    if (!p) return null;
    if (/^(import|from|const|let|function|return|export|type|interface|if|else|new|public|private|protected|class|void|int|long|String|List|Map|SELECT|FROM|WHERE|INSERT|UPDATE|DELETE|INTO|VALUES|AND|OR|JOIN|ON|ORDER|BY|LIMIT|SET|CREATE|TABLE)$/i.test(p)) {
      return (
    ...
    if (p.startsWith("#{") || p.startsWith("${")) {
      return (
        <span key={i} className="text-peach">
          {p}
        </span>
      );
    }
    if (/^<\/?[A-Za-z]/.test(p) || p.startsWith("@")) {
      return (
        <span key={i} className="text-sapphire">
          {p}
        </span>
      );
    }
